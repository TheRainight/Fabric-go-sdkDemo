...
func InstallAndInstantiateCC(sdk *fabsdk.FabricSDK, info *InitInfo) (*channel.Client, error) {
    ...
    ccPolicy := cauthdsl.SignedByAnyMember([]string{"FBIMSP"})
// 
    instantiateCCReq := resmgmt.InstantiateCCRequest{Name: info.ChaincodeID, Path: info.ChaincodePath, Version: ChaincodeVersion, Args: [][]byte{[]byte("init"),[]byte("AAA"),[]byte("100"),[]byte("BBB"),[]byte("100")}, Policy: ccPolicy}
